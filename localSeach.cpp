...
vector<vector<short>> localSearch(vector<vector<short>> data,
    ...
    while (l <= lmax) {
        int choice = rand() % 2;
        if (choice == 0) { // moveRow()
            int first_row = rand() % (solution[0].size() + 1);
            int second_row = rand() % (solution[0].size() + 1);
            if (solution[0][first_row] == solution[0][second_row]) {
                second_row = rand() % (solution[0].size() + 1);
            }
            moveRow(solution, first_row, second_row);
        } else { // moveColumns()
            int first_columns = rand() % (solution[1].size() + 1);
            int second_columns = rand() % (solution[1].size() + 1);
            if (solution[0][first_columns] == solution[0][second_columns]) {
            second_columns = rand() % (solution[1].size() + 1);
        }
...
void moveRow (vector<vector<short>> solution,
  ...
  int id_cluster = solution[0][second_row];
...
void moveColumns(vector<vector<short>> solution,
  ...
  int id_cluster = solution[1][second_columns];
